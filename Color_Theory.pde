...
void setup() {
  ...
  //gif
  gif = new PImage [179];
  int stargifCounter = 0;
  String zeroes = "";
  while (stargifCounter < 179) {
    if (stargifCounter / 10 < 1) {
     zeroes = "00"; 
    } else if (stargifCounter / 100 < 1) {
     zeroes = "0"; 
    } else zeroes = "";
    gif[stargifCounter] = loadImage("frame_" + zeroes + stargifCounter + "_delay-0.03s.png");
    gif[stargifCounter].resize(400, 400);
    stargifCounter++;
  }
}
